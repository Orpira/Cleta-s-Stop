...
import { DEFAULT_TRIKI_SETTINGS } from '@/lib/games/triki/gameLogic';
...
export default function TrikiHome() {
  ...
  async function handleCreate() {
    ...
    setError('');
    try {
      const { room } = await createRoom(GAME_TYPE, nickname, DEFAULT_TRIKI_SETTINGS);
      router.push(`/juegos/triki/sala/${room.code}`);
    } catch (e: any) {
  ...
  return (
    <Sheet brand="orpira.es">
      ...
      <p className="subtitle">
        El clásico 3 en raya — dos jugadores, por turnos, sin reloj.
      </p>
      ...
      >
        {loading ? 'Un momento…' : mode === 'create' ? 'Crear sala' : 'Unirme'}
      </Button>
    </Sheet>
  );
}
